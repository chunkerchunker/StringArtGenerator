...
void freeGenerator(StringArtGenerator* gen) {
  ...
  for (int i = 0; i < cacheSize; i++) {
    if (gen->lineCacheX[i])
      free(gen->lineCacheX[i]);
    if (gen->lineCacheY[i])
      free(gen->lineCacheY[i]);
  }
  free(gen->lineCacheX);
  free(gen->lineCacheY);
...
void loadAndProcessImage(StringArtGenerator* gen) {
  ...
  unsigned char* img =
      stbi_load(gen->config.inputFile, &width, &height, &channels, 0);
...
int findOptimalWeight(StringArtGenerator* gen) {
  // Binary search for MAXIMUM weight that produces < maxLines
  // Lower weight = more lines, Higher weight = fewer lines
  // Goal: Find the maximum weight that produces < maxLines (avoid hitting the
  // algorithm limit)
  int low = 1;
  int high = 200;
  ...
  memcpy(savedSourceImage, gen->sourceImage, gen->imgSizeSq * sizeof(double));
  ...
  while (low <= high) {
    ...
    // Restore original image data for consistent testing
    memcpy(gen->sourceImage, savedSourceImage, gen->imgSizeSq * sizeof(double));
    ...
    if (lineCount - 1 < targetMaxLines) {
      // This weight produces acceptable line count, try higher weight
      bestWeight = mid;
      low = mid + 1;
    } else {
      // At or above limit, try lower weight
      high = mid - 1;
    }
...
void generateOutputImage(StringArtGenerator* gen,
                         int* lineSequence,
                         int lineCount) {
  ...
  stbi_write_png(gen->config.outputFile, outSize, outSize, 4, image,
                 outSize * 4);
  free(image);
}
...
int main(int argc, char* argv[]) {
  ...
  if (config.autoWeight) {
    ...
    int optimalWeight = findOptimalWeight(&gen);
    config.outputWeight =
        (config.outputWeight == 0) ? optimalWeight : config.outputWeight;

    // Calculate final lines with optimal weight
    lineSequence = calculateLines(&gen, &lineCount);
  } else {
    lineSequence = calculateLines(&gen, &lineCount);
  }
